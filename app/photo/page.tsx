...
export default function PhotoPage() {
  ...
  const gridInfoRef = useRef({
    ...
    get cellHeight() {
      return this.imgHeight + this.gap;
    }, // 动态计算单元格高度
  });
  ...
  useEffect(() => {
    ...
    const fetchPixabayImages = async () => {
      try {
        ...
        // 初始化可用图片索引数组
        availableImageIndicesRef.current = Array.from(
        ...
        for (let i = availableImageIndicesRef.current.length - 1; i > 0; i--) {
          ...
          [
            availableImageIndicesRef.current[i],
            availableImageIndicesRef.current[j],
          ] = [
            availableImageIndicesRef.current[j],
            availableImageIndicesRef.current[i],
          ];
  ...
  const generateInitialImages = () => {
    ...
    // 生成围绕中心点的网格
    for (
      let gridY = centerGridY - Math.floor(rows / 2);
      gridY <= centerGridY + Math.floor(rows / 2);
      gridY++
    ) {
      for (
        let gridX = centerGridX - Math.floor(cols / 2);
        gridX <= centerGridX + Math.floor(cols / 2);
        gridX++
      ) {
  ...
  const drawImages = () => {
    ...
    // 计算可视区域边界
    const viewportLeft =
      -offsetRef.current.x - (viewportWidth * bufferFactor) / 2;
    const viewportRight =
      -offsetRef.current.x + (viewportWidth * bufferFactor) / 2;
    const viewportTop =
      -offsetRef.current.y - (viewportHeight * bufferFactor) / 2;
    ...
    imagesRef.current.forEach((img) => {
      ...
      if (cachedImage && cachedImage.complete) {
        ...
        if (widthRatio > heightRatio) {
          ...
          sourceY = Math.max(
            0,
            Math.min(totalExcessHeight, totalExcessHeight * factor)
          );
        } else {
          // 高度比例大，以高度为基准，裁剪宽度
          ...
          sourceX = Math.max(
            0,
            Math.min(totalExcessWidth, totalExcessWidth * factor)
          );
        }
  ...
  const isImageExistAtGrid = (gridX: number, gridY: number) => {
    return imagesRef.current.some(
      (img) => img.gridX === gridX && img.gridY === gridY
    );
  };
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // 检查是否为右键点击
    if (e.button === 2) return; // 右键点击时不执行任何操作
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (!isDraggingRef.current || isPreviewingRef.current || showPreview)
      return; // 如果不在拖拽状态，则返回
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLCanvasElement>) => {
    ...
    if (deltaTime > 0) {
      velocityRef.current = {
        x:
          ((e.touches[0].clientX - lastMousePosRef.current.x) / deltaTime) * 16,
        y:
          ((e.touches[0].clientY - lastMousePosRef.current.y) / deltaTime) * 16,
      };
    }
      ...
      {isLoading ? (
        ...
        <>
          ...
          {showPreview && previewImage && (
            <ImagePreview image={previewImage} onClose={handleClosePreview} />
          )}
