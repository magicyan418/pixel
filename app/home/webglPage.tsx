...
export default function WebglPage() {
  ...
  useEffect(() => {
    ...
    const htmlClasses = ["overflow-hidden", "bg-black"];
    const bodyClasses = ["m-0", "fixed", "w-full", "h-full", "bg-black", "overflow-hidden"];
    document.documentElement.classList.add(...htmlClasses);
    document.body.classList.add(...bodyClasses);

    ("use strict");
    console.log(
      `%c PIXEL %c magicyan418 `,
    ...
    console.log("https://github.com/magicyan418/pixel");
  ...
  return (
    <div className="fixed inset-0 select-none bg-black">
      ...
      <div className="pointer-events-none fixed inset-0 text-center text-white mix-blend-difference">
        <div className="absolute left-0 right-0 top-1/4">
          <h1 className="aqua-ink-title m-0 font-sans text-[3rem] leading-none text-white sm:text-[6rem]">
            MAGICYAN
          </h1>
          <h2 className="font-sans text-base font-normal sm:text-xl opacity-80 mt-4">
            所有一切刚刚好
          </h2>
        </div>

        <footer className="absolute inset-x-0 bottom-0 px-8 py-5 text-white">
          <div className="flex flex-wrap items-center justify-center gap-3 text-sm">
            <span className="text-xs">
              &copy; <time dateTime={String(year)}>{year}</time> PIXEL
            </span>
            <span className="hidden select-none sm:inline" aria-hidden="true">
              |
            </span>
            <a
              href="https://github.com/magicyan/pixel"
              target="_blank"
              rel="noopener noreferrer"
              className="pointer-events-auto inline-flex items-center gap-2 text-white transition hover:-translate-y-px"
              aria-label="View PIXEL source on GitHub"
            >
              <svg
             ...
             1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"
              ...
              <span className="hidden text-xs sm:inline">Source Code</span>
            </a>
          </div>
        </footer>
      </div>
      <style jsx global>{`
        .aqua-ink-title {
          animation: aqua-ink-glitch 0.5s linear infinite;
        }

        @keyframes aqua-ink-glitch {
          0%,
          100% {
